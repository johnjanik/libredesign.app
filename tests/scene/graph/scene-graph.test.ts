...
describe('SceneGraph', () => {
  ...
  describe('node access', () => {
    ...
    it('getNode returns null for non-existent ID', () => {
      const result = graph.getNode('non-existent' as unknown as import('@core/types/common').NodeId);
    ...
    it('hasNode returns correct boolean', () => {
      ...
      expect(graph.hasNode('non-existent' as unknown as import('@core/types/common').NodeId)).toBe(false);
    });
  ...
  describe('node creation', () => {
    ...
    it('emits node:created event', () => {
      ...
      expect(handler).toHaveBeenCalledWith(
        expect.objectContaining({ nodeType: 'FRAME' })
      );
    });
  ...
  describe('node modification', () => {
    ...
    it('updateNode modifies node properties', () => {
      ...
      expect((frame as NodeData & { x: number }).x).toBe(100);
      expect((frame as NodeData & { y: number }).y).toBe(200);
    });
    ...
    it('throws when updating non-existent node', () => {
      expect(() => {
        graph.updateNode('non-existent' as unknown as import('@core/types/common').NodeId, { name: 'Test' });
      }).toThrow('not found');
    });
  });
  ...
  describe('traversal', () => {
    ...
    it('traverse can skip subtrees', () => {
      ...
      const visited: string[] = [];
      graph.traverse((node): boolean | void => {
        visited.push(node.name);
        if (node.type === 'FRAME') return false; // Skip frame's children
        return undefined;
      });
