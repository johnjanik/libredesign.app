...
} from '@persistence/serialization/document-serializer';
...
describe('DocumentSerializer', () => {
  ...
  describe('serialize', () => {
    ...
    it('preserves node hierarchy', () => {
      ...
      const vectorNode = data.nodes.find((n: { data: { name: string } }) => n.data.name === 'Vector');
    ...
    it('preserves node child indices', () => {
      ...
      const frameNodes = data.nodes.filter((n: { data: { type: string } }) => n.data.type === 'FRAME');
      const indices = frameNodes.map((n: { childIndex: number }) => n.childIndex).sort();
  ...
  describe('round-trip serialization', () => {
    it('preserves document structure', () => {
      ...
      const frame1Node = parsed.nodes.find((n: { data: { name: string } }) => n.data.name === 'Frame 1');
      const vector1Node = parsed.nodes.find((n: { data: { name: string } }) => n.data.name === 'Vector 1');
    ...
    it('preserves node properties', () => {
      ...
      const frameNode = parsed.nodes.find((n: { data: { name: string } }) => n.data.name === 'Styled Frame');
