...
describe('MaskRenderer', () => {
    ...
    beforeEach(() => {
        ...
        pool = createMockPool();
        renderer = new MaskRenderer(ctx, shaders, pool);
    });
    ...
    describe('applyMask', () => {
        it('composites content with mask', () => {
            ...
            renderer.applyMask(content, mask);
        ...
        it('sets mask type uniform', () => {
            const content = createMockRenderTarget();
            const mask = createMockRenderTarget();
            renderer.applyMask(content, mask, { type: 'luminance' });
            expect(ctx.gl.uniform1i).toHaveBeenCalled();
        });
        it('sets opacity uniform', () => {
            const content = createMockRenderTarget();
            const mask = createMockRenderTarget();
            renderer.applyMask(content, mask, { opacity: 0.75 });
    ...
    describe('renderMaskedShape', () => {
        it('renders shape to mask texture', () => {
            ...
            const viewProjection = [1, 0, 0, 1, 0, 0];
            const result = renderer.renderMaskedShape(content, vertices, indices, transform, viewProjection);
        ...
        it('applies feather to shape mask', () => {
            ...
            const transform = [1, 0, 0, 1, 0, 0];
            const viewProjection = [1, 0, 0, 1, 0, 0];
            renderer.renderMaskedShape(content, vertices, indices, transform, viewProjection, { feather: 8 });
            expect(shaders.use).toHaveBeenCalledWith('blur');
        });
    });
    describe('cancelMask', () => {
    ...
    describe('mask types', () => {
        const maskTypes = ['alpha', 'luminance', 'inverse-alpha', 'inverse-luminance'];
        it.each(maskTypes)('supports %s mask type', (type) => {
            const content = createMockRenderTarget();
            const mask = createMockRenderTarget();
            expect(() => {
                renderer.applyMask(content, mask, { type });
            }).not.toThrow();
        });
    });
});
describe('createMaskRenderer', () => {
    it('creates a mask renderer', () => {
        ...
        const renderer = createMaskRenderer(ctx, shaders, pool);
        expect(renderer).toBeInstanceOf(MaskRenderer);
    });
});
//# sourceMappingURL=mask-renderer.test.js.map