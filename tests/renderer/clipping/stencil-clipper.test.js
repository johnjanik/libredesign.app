...
describe('StencilClipper', () => {
    ...
    describe('initialization', () => {
        ...
        it('sets up vertex attributes', () => {
            ...
            expect(ctx.gl.vertexAttribPointer).toHaveBeenCalledWith(0, 2, GL.FLOAT, false, 0, 0);
        });
    ...
    describe('beginClip', () => {
        ...
        it('re-enables color writes after stencil write', () => {
            ...
            const calls = ctx.gl.colorMask.mock.calls;
        ...
        it('sets matrix uniforms', () => {
            ...
            const viewProjection = [0.5, 0, 0, 0.5, 0, 0];
            clipper.beginClip(vertices, indices, transform, viewProjection);
            expect(ctx.gl.uniformMatrix3fv).toHaveBeenCalledTimes(2);
        });
        it('draws clip path', () => {
            clipper.beginClip(vertices, indices, IDENTITY, IDENTITY);
            expect(ctx.drawElements).toHaveBeenCalledWith(GL.TRIANGLES, indices.length, GL.UNSIGNED_SHORT, 0);
        });
    ...
    describe('beginRectClip', () => {
        it('creates rectangle vertices', () => {
            ...
            expect(ctx.gl.bufferData).toHaveBeenCalledWith(GL.ARRAY_BUFFER, expect.any(Float32Array), GL.DYNAMIC_DRAW);
        });
        it('creates correct triangle indices', () => {
            clipper.beginRectClip(0, 0, 100, 100, IDENTITY, IDENTITY);
            // Should create two triangles
            const bufferDataCalls = ctx.gl.bufferData.mock.calls;
            const indicesCall = bufferDataCalls.find(call => call[0] === GL.ELEMENT_ARRAY_BUFFER);
            expect(indicesCall).toBeDefined();
            const indices = indicesCall[1];
    ...
    describe('matrix transformation', () => {
        ...
        it('converts Matrix2x3 to 3x3 matrix for WebGL', () => {
            ...
            const matrixCalls = ctx.gl.uniformMatrix3fv.mock.calls;
            const transformCall = matrixCalls.find(call => JSON.stringify(call[2]).includes('5') && JSON.stringify(call[2]).includes('6'));
...
//# sourceMappingURL=stencil-clipper.test.js.map