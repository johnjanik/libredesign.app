...
import { ShaderManager, createShaderManager, type ShaderSource } from '@renderer/shaders/shader-manager';
...
describe('ShaderManager', () => {
  ...
  describe('registerShader', () => {
    it('registers custom shaders', () => {
      const customSource: ShaderSource = {
        vertex: 'void main() {}',
        fragment: 'void main() {}',
        uniforms: ['uCustom'],
        attributes: ['aCustom'],
    ...
    it('registers shader without uniforms or attributes', () => {
      const minimalSource: ShaderSource = {
        vertex: 'void main() {}',
        fragment: 'void main() {}',
      };
  ...
  describe('getProgram', () => {
    ...
    it('handles missing uniform locations gracefully', () => {
      (ctx.gl.getUniformLocation as ReturnType<typeof vi.fn>).mockReturnValue(null);
    ...
    it('handles invalid attribute locations gracefully', () => {
      (ctx.gl.getAttribLocation as ReturnType<typeof vi.fn>).mockReturnValue(-1);
  ...
  describe('compilation errors', () => {
    it('throws on shader creation failure', () => {
      (ctx.gl.createShader as ReturnType<typeof vi.fn>).mockReturnValue(null);

      expect(() => manager.getProgram('fill')).toThrow('Failed to create shader');
    });

    it('throws on vertex shader compilation failure', () => {
      (ctx.gl.getShaderParameter as ReturnType<typeof vi.fn>).mockReturnValueOnce(false);
      (ctx.gl.getShaderInfoLog as ReturnType<typeof vi.fn>).mockReturnValueOnce('Vertex error');
    ...
    it('throws on fragment shader compilation failure', () => {
      (ctx.gl.getShaderParameter as ReturnType<typeof vi.fn>)
        .mockReturnValueOnce(true)  // vertex succeeds
        .mockReturnValueOnce(false); // fragment fails
      (ctx.gl.getShaderInfoLog as ReturnType<typeof vi.fn>).mockReturnValueOnce('Fragment error');
    ...
    it('throws on program creation failure', () => {
      (ctx.gl.createProgram as ReturnType<typeof vi.fn>).mockReturnValue(null);

      expect(() => manager.getProgram('fill')).toThrow('Failed to create program');
    });

    it('throws on link failure', () => {
      (ctx.gl.getProgramParameter as ReturnType<typeof vi.fn>).mockReturnValue(false);
      (ctx.gl.getProgramInfoLog as ReturnType<typeof vi.fn>).mockReturnValue('Link error');
