...
import {
  ...
  HSL_CONVERSION_GLSL,
  type BlendFactors,
} from '@renderer/blending/blend-mode-map';
...
describe('getBlendFactors', () => {
  ...
  describe('GL-based blend modes', () => {
    ...
    it('returns default factors for unknown blend mode', () => {
      const factors = getBlendFactors(gl, 'UNKNOWN_MODE' as BlendMode);
...
describe('requiresShaderBlend', () => {
  ...
  it('returns false for unknown mode', () => {
    expect(requiresShaderBlend('UNKNOWN' as BlendMode)).toBe(false);
  });
});
...
describe('getBlendModeGLSL', () => {
  ...
  describe('default mode', () => {
    it('returns normal blend function for unknown mode', () => {
      const glsl = getBlendModeGLSL('UNKNOWN' as BlendMode);
