...
import type { ShaderManager } from '@renderer/shaders/shader-manager';
import type { RenderTargetPool } from '@renderer/effects/render-target-pool';
import type { RenderTarget } from '@renderer/effects/render-target';
import type { InnerShadowConfig } from '@renderer/effects/inner-shadow-effect';
...
const createMockRenderTarget = (width = 256, height = 256): RenderTarget => ({
  width,
  height,
...
const createMockPool = (): RenderTargetPool => {
  ...
  return {
    ...
    clear: vi.fn(),
    dispose: vi.fn(),
  } as unknown as RenderTargetPool;
};

// Default inner shadow config for tests
const defaultShadowConfig: InnerShadowConfig = {
...
describe('InnerShadowEffectRenderer', () => {
  ...
  let renderer: InnerShadowEffectRenderer;
  ...
  describe('apply', () => {
    ...
    it('sets shadow offset uniform', () => {
      ...
      const config: InnerShadowConfig = {
        ...defaultShadowConfig,
        offset: { x: 5, y: 10 },
      };
      ...
      expect(ctx.gl.uniform2f).toHaveBeenCalledWith(
        expect.anything(),
        5 / 200,  // offset.x / width
    ...
    it('sets shadow color uniform', () => {
      ...
      expect(ctx.gl.uniform4f).toHaveBeenCalledWith(
        expect.anything(),
        0.8,  // r
        0.2,  // g
        0.1,  // b
    ...
    it('does not release source when releaseSource is false', () => {
      ...
      const releaseCalls = (pool.release as ReturnType<typeof vi.fn>).mock.calls;
  ...
  describe('renderShadowOnly', () => {
    it('renders shadow without compositing', () => {
      ...
      const compositeCalls = (shaders.use as ReturnType<typeof vi.fn>).mock.calls
  ...
  describe('applyMultiple', () => {
    ...
    it('applies multiple shadows in sequence', () => {
      ...
      const innerShadowCalls = (shaders.use as ReturnType<typeof vi.fn>).mock.calls
