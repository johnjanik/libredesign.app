...
describe('RenderTarget', () => {
    ...
    describe('initialization', () => {
        ...
        it('sets texture parameters', () => {
            ...
            expect(ctx.gl.texParameteri).toHaveBeenCalledWith(GL.TEXTURE_2D, GL.TEXTURE_MIN_FILTER, GL.LINEAR);
            expect(ctx.gl.texParameteri).toHaveBeenCalledWith(GL.TEXTURE_2D, GL.TEXTURE_MAG_FILTER, GL.LINEAR);
            expect(ctx.gl.texParameteri).toHaveBeenCalledWith(GL.TEXTURE_2D, GL.TEXTURE_WRAP_S, GL.CLAMP_TO_EDGE);
            expect(ctx.gl.texParameteri).toHaveBeenCalledWith(GL.TEXTURE_2D, GL.TEXTURE_WRAP_T, GL.CLAMP_TO_EDGE);
            target.dispose();
        });
        it('allocates texture storage', () => {
            const target = new RenderTarget(ctx, { width: 512, height: 512 });
            expect(ctx.gl.texStorage2D).toHaveBeenCalledWith(GL.TEXTURE_2D, 1, GL.RGBA8, 512, 512);
            target.dispose();
        });
        it('attaches texture to framebuffer', () => {
            new RenderTarget(ctx, { width: 100, height: 100 });
            expect(ctx.gl.framebufferTexture2D).toHaveBeenCalledWith(GL.FRAMEBUFFER, GL.COLOR_ATTACHMENT0, GL.TEXTURE_2D, expect.anything(), 0);
        });
        ...
        it('throws if framebuffer is incomplete', () => {
            ctx.gl.checkFramebufferStatus.mockReturnValue(0);
            expect(() => {
                new RenderTarget(ctx, { width: 100, height: 100 });
        ...
        it('accepts custom filter and wrap options', () => {
            ...
            expect(ctx.gl.texParameteri).toHaveBeenCalledWith(GL.TEXTURE_2D, GL.TEXTURE_MIN_FILTER, GL.NEAREST);
            target.dispose();
        });
    ...
    describe('texture binding', () => {
        it('binds texture to specified unit', () => {
            ...
            expect(ctx.bindTexture).toHaveBeenCalledWith(GL.TEXTURE_2D, target.texture, 2);
            target.dispose();
        });
...
//# sourceMappingURL=render-target.test.js.map