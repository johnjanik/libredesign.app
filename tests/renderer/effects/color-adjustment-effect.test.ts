...
import {
  colorAdjustment,
  isColorAdjustmentEffect,
} from '@core/types/effect';
import type { ColorAdjustmentEffect, Effect } from '@core/types/effect';
...
describe('isColorAdjustmentEffect', () => {
  it('should return true for color adjustment effects', () => {
    const effect: Effect = colorAdjustment({ hue: 10 });
...
describe('ColorAdjustmentConfig', () => {
  it('should detect noop configurations', () => {
    ...
    const isNoop = (config: ColorAdjustmentEffect) => {
      const epsilon = 0.01;
      return (
