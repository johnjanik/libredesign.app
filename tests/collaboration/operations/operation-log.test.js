...
import { createOperationLog } from '@collaboration/operations/operation-log';
import { createTimestamp, generateOperationId, } from '@collaboration/operations/operation-types';
describe('OperationLog', () => {
    let log;
    const createInsertOp = (counter, nodeId = 'node-1') => ({
    ...
    const createSetPropertyOp = (counter, nodeId = 'node-1', path = ['name']) => ({
        id: generateOperationId('client-1'),
        type: 'SET_PROPERTY',
        timestamp: createTimestamp(counter, 'client-1'),
        nodeId: nodeId,
    ...
    const createDeleteOp = (counter, nodeId = 'node-1') => ({
        id: generateOperationId('client-1'),
        type: 'DELETE_NODE',
        timestamp: createTimestamp(counter, 'client-1'),
        nodeId: nodeId,
    ...
    describe('getOperationsForNode', () => {
        it('returns all operations for a specific node', () => {
            ...
            const ops = log.getOperationsForNode('node-1');
            expect(ops.length).toBe(3);
            expect(ops.every(op => op.nodeId === 'node-1')).toBe(true);
        });
        it('returns empty array for unknown node', () => {
            log.append(createInsertOp(1, 'node-1'));
            const ops = log.getOperationsForNode('unknown');
            expect(ops.length).toBe(0);
        });
    });
...
//# sourceMappingURL=operation-log.test.js.map