...
} from '@collaboration/presence/presence-manager';
import type { NodeId } from '@core/types/common';

describe('PresenceManager', () => {
  let manager: PresenceManager;

  const defaultConfig: PresenceManagerConfig = {
  ...
  describe('local presence updates', () => {
    ...
    it('updates local selection', () => {
      const selection = ['node-1' as NodeId, 'node-2' as NodeId];
  ...
  describe('remote presence updates', () => {
    ...
    it('emits selectionChanged event on selection update', () => {
      ...
      const selection = ['node-1' as NodeId];
      manager.updateRemotePresence('remote-1', {
        userName: 'Alice',
        selection,
      });
  ...
  describe('getActiveSelections', () => {
    it('returns selections for users with selected nodes', () => {
      const selection1 = ['node-1' as NodeId, 'node-2' as NodeId];
      const selection2 = ['node-3' as NodeId];
  ...
  describe('node selection queries', () => {
    beforeEach(() => {
      manager.updateRemotePresence('remote-1', {
        userName: 'Alice',
        selection: ['node-1' as NodeId, 'node-2' as NodeId],
      });
      manager.updateRemotePresence('remote-2', {
        userName: 'Bob',
        selection: ['node-2' as NodeId, 'node-3' as NodeId],
      });
    });

    it('checks if node is selected by remote user', () => {
      expect(manager.isNodeSelectedByRemote('node-1' as NodeId)).toBe(true);
      expect(manager.isNodeSelectedByRemote('node-2' as NodeId)).toBe(true);
      expect(manager.isNodeSelectedByRemote('node-4' as NodeId)).toBe(false);
    });

    it('gets clients selecting a node', () => {
      expect(manager.getClientsSelectingNode('node-1' as NodeId)).toEqual(['remote-1']);
      expect(manager.getClientsSelectingNode('node-2' as NodeId)).toEqual(['remote-1', 'remote-2']);
      expect(manager.getClientsSelectingNode('node-3' as NodeId)).toEqual(['remote-2']);
      expect(manager.getClientsSelectingNode('node-4' as NodeId)).toEqual([]);
    });
  });

  describe('cleanup', () => {
    it('removes expired presences on cleanup interval', () => {
      const shortConfig: PresenceManagerConfig = {
        localClientId: 'local',
        localUserName: 'Local',
