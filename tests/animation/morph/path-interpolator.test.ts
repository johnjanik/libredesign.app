...
} from '../../../src/animation/morph/path-interpolator';
import {
  checkCompatibility,
  countPoints,
} from '../../../src/animation/morph/path-compatibility';
import {
  extractPoints,
  normalizePathPoints,
} from '../../../src/animation/morph/point-matching';

// Helper to create simple paths
function createSquarePath(size: number, x = 0, y = 0): VectorPath {
  return {
    commands: [
      { type: 'M', x: x, y: y },
      { type: 'L', x: x + size, y: y },
...
function createTrianglePath(size: number, x = 0, y = 0): VectorPath {
  return {
    commands: [
...
describe('checkCompatibility', () => {
  ...
  it('should return incompatible for large point differences', () => {
    ...
    const commandsList: Array<{ type: string; x?: number; y?: number }> = [{ type: 'M', x: 0, y: 0 }];
    for (let i = 1; i <= 60; i++) {
      commandsList.push({ type: 'L', x: i * 10, y: Math.sin(i) * 50 });
    }
    commandsList.push({ type: 'Z' });

    const complex: VectorPath = { commands: commandsList as VectorPath['commands'], windingRule: 'NONZERO' };
