...
describe('AnimationPlayer', () => {
  ...
  let nodes: Map<NodeId, Record<string, unknown>>;
  ...
  beforeEach(() => {
    ...
    // Set up initial node
    nodes.set('test-node' as NodeId, { x: 0, y: 0, opacity: 1 });
  });
  ...
  it('should register and retrieve animations', () => {
    const animation = createAnimation(
      [createAnimatedProperty('x', [{ time: 0, value: 0 }, { time: 1, value: 100 }])],
      { duration: 1000 }
    );
  ...
  it('should play an animation', () => {
    ...
    player.registerAnimation(animation);
    const instanceId = player.play(animation.id, 'test-node' as NodeId);
  ...
  it('should throw when playing non-existent animation', () => {
    expect(() => {
      player.play('non-existent', 'test-node' as NodeId);
  ...
  it('should emit start event', () => {
    ...
    player.registerAnimation(animation);
    player.play(animation.id, 'test-node' as NodeId);
  ...
  it('should stop all animations', () => {
    ...
    const animation2 = createAnimation(
      [createAnimatedProperty('y', [{ time: 0, value: 0 }, { time: 1, value: 100 }])],
      { duration: 1000 }
    );
    ...
    const id1 = player.play(animation1.id, 'test-node' as NodeId);
    const id2 = player.play(animation2.id, 'test-node' as NodeId);
  ...
  it('should track running count', () => {
    ...
    const id1 = player.play(animation.id, 'test-node' as NodeId);
