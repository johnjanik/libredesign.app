...
import { lineLineIntersection, lineBezierIntersection, bezierBezierIntersection, evaluateBezier, bezierDerivative, bezierBounds, splitBezier, flattenBezier, solveQuadratic, pointsEqual, signedArea, isClockwise, distance, distanceSquared, EPSILON, } from '@core/geometry/boolean';
describe('lineLineIntersection', () => {
    it('finds intersection of perpendicular lines', () => {
        const result = lineLineIntersection({ x: 0, y: 5 }, { x: 10, y: 5 }, { x: 5, y: 0 }, { x: 5, y: 10 });
    ...
    it('finds intersection at segment start', () => {
        const result = lineLineIntersection({ x: 0, y: 0 }, { x: 10, y: 0 }, { x: 0, y: 0 }, { x: 0, y: 10 });
        expect(result).not.toBeNull();
    ...
    it('handles near-parallel lines correctly', () => {
        const result = lineLineIntersection({ x: 0, y: 0 }, { x: 100, y: 0.001 }, { x: 0, y: 10 }, { x: 100, y: 10.001 });
        expect(result).toBeNull();
    });
    it('finds intersection for diagonal lines', () => {
        const result = lineLineIntersection({ x: 0, y: 0 }, { x: 100, y: 100 }, { x: 100, y: 0 }, { x: 0, y: 100 });
        expect(result).not.toBeNull();
...
describe('evaluateBezier', () => {
    ...
    it('handles linear bezier (control points on line)', () => {
        ...
        const linear = evaluateBezier({ x: 0, y: 0 }, { x: 33, y: 0 }, { x: 66, y: 0 }, { x: 100, y: 0 }, 0.5);
...
describe('flattenBezier', () => {
    ...
    it('straight line produces minimal points', () => {
        const straight = flattenBezier({ x: 0, y: 0 }, { x: 33, y: 0 }, { x: 66, y: 0 }, { x: 100, y: 0 });
...
describe('bezierBezierIntersection', () => {
    it('finds intersection of crossing curves', () => {
        const results = bezierBezierIntersection({ x: 0, y: 0 }, { x: 0, y: 100 }, { x: 100, y: 100 }, { x: 100, y: 0 }, { x: 0, y: 100 }, { x: 100, y: 100 }, { x: 100, y: 0 }, { x: 0, y: 0 });
        expect(results.length).toBeGreaterThanOrEqual(1);
    });
    it('returns empty for non-intersecting curves', () => {
        const results = bezierBezierIntersection({ x: 0, y: 0 }, { x: 10, y: 10 }, { x: 20, y: 10 }, { x: 30, y: 0 }, { x: 0, y: 100 }, { x: 10, y: 110 }, { x: 20, y: 110 }, { x: 30, y: 100 });
        expect(results.length).toBe(0);
    });
});
