...
import {
  ...
  createStrokeOutline,
} from '@core/geometry/path-offset';
import type { VectorPath } from '@core/types/geometry';

// Helper to create a rectangular path
function createRectPath(x: number, y: number, w: number, h: number): VectorPath {
  return {
    windingRule: 'NONZERO',
    commands: [
      { type: 'M', x, y },
...
function approximatePathArea(path: VectorPath): number {
...
describe('offsetPath', () => {
  ...
  describe('path with curves', () => {
    it('handles path with bezier curves', () => {
      const pathWithCurve: VectorPath = {
        windingRule: 'NONZERO',
        commands: [
          { type: 'M', x: 0, y: 0 },
          { type: 'L', x: 100, y: 0 },
