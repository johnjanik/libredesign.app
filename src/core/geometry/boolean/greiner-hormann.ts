...
import { Polygon, Vertex, createPolygonsFromPath, polygonsToVectorPath } from './polygon';
...
const DEFAULT_CONFIG: Required<BooleanConfig> = {
...
export function computeBooleanOperation(
  ...
  subjectPaths: VectorPath[],
  clipPaths: VectorPath[],
  ...
  try {
    ...
    const subjectPolygons: Polygon[] = [];
    for (const path of subjectPaths) {
      subjectPolygons.push(
        ...createPolygonsFromPath(path, 'subject', cfg.flattenTolerance)
      );
    }

    const clipPolygons: Polygon[] = [];
    ...
    for (const subject of subjectPolygons) {
      let currentResults: Polygon[] = [subject];

      for (const clip of clipPolygons) {
        const nextResults: Polygon[] = [];

        for (const current of currentResults) {
          const opResults = processPolygonPair(
            current.clone(),
            clip.clone(),
            operation,
            cfg
    ...
    const resultPath = polygonsToVectorPath(
      resultPolygons,
      subjectPaths[0]?.windingRule || 'NONZERO'
    );
 ...
 */
function processPolygonPair(
  subject: Polygon,
  clip: Polygon,
  operation: BooleanOperation,
  config: Required<BooleanConfig>
): Polygon[] {
 ...
 */
export function findAndInsertIntersections(
  subject: Polygon,
  clip: Polygon,
  tolerance: number = EPSILON
): void {
  if (!subject.first || !clip.first) return;

  // Collect edges before modification
  const subjectEdges: Array<{ v1: Vertex; v2: Vertex }> = [];
  for (const [v1, v2] of subject.edges()) {
    subjectEdges.push({ v1, v2 });
  }

  const clipEdges: Array<{ v1: Vertex; v2: Vertex }> = [];
  ...
  for (const subjectEdge of subjectEdges) {
    for (const clipEdge of clipEdges) {
      const intersection = lineLineIntersection(
        subjectEdge.v1.point,
        subjectEdge.v2.point,
        clipEdge.v1.point,
        clipEdge.v2.point
      );
 ...
 */
function isAtEndpoint(t: number, tolerance: number): boolean {
 ...
 */
function handleEndpointIntersection(
  subjectEdge: { v1: Vertex; v2: Vertex },
  clipEdge: { v1: Vertex; v2: Vertex },
  intersection: { point: Point; t1: number; t2: number },
  tolerance: number
): void {
  // Determine which vertices are at the intersection
  let subjectVertex: Vertex | null = null;
  let clipVertex: Vertex | null = null;
