 ...
 */
export function classifyPoint(point, polygon) {
 ...
 */
export function isPointOnSegment(point, segStart, segEnd, tolerance = EPSILON) {
    ...
    const cross = Math.abs((point.x - segStart.x) * dy - (point.y - segStart.y) * dx);
    ...
    for (let i = 0; i < edges.length; i++) {
        for (let j = i + 2; j < edges.length; j++) {
            ...
            if (intersection) {
                ...
                const isAtEndpoint = (intersection.t1 < EPSILON || intersection.t1 > 1 - EPSILON) &&
 ...
 */
export function markEntryExitPoints(subject, clip, operation) {
...
function markEntryExitForPolygon(polygon, other, operation, isSubject) {
    if (!polygon.first)
        return;
    // Find first non-intersection vertex to determine initial inside/outside state
    let firstNonIntersection = null;
 ...
 */
function getEntryStatus(isInside, isSubject, operation) {
    switch (operation) {
        case 'UNION':
