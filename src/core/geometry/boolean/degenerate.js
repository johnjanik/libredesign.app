 ...
 * - Identical polygons
 */
import { createPolygonFromPoints } from './polygon';
 ...
 */
export function isDegenerateCase(subject, clip) {
 ...
 */
export function handleDegenerateCases(subject, clip, operation) {
 ...
 */
export function isZeroArea(polygon) {
 ...
 */
export function hasSharedVertices(a, b) {
 ...
 */
export function hasCoincidentEdges(a, b) {
 ...
 */
function areEdgesCoincident(a1, a2, b1, b2) {
 ...
 */
export function arePolygonsIdentical(a, b) {
    if (a.count !== b.count)
        return false;
 ...
 */
function handleZeroAreaSubject(clip, operation) {
    switch (operation) {
        case 'UNION':
            return [clip.clone()];
        case 'SUBTRACT':
 ...
 */
function handleZeroAreaClip(subject, operation) {
    switch (operation) {
        case 'UNION':
 ...
 */
function handleIdenticalPolygons(polygon, operation) {
    switch (operation) {
        case 'UNION':
 ...
 */
function handleContainment(subject, clip, operation, subjectInClip) {
    if (subjectInClip) {
 ...
 */
export function perturbPolygon(polygon, amount = EPSILON * 10) {
    const points = [];
 ...
 */
export function snapNearbyVertices(subject, clip, tolerance = EPSILON * 10) {
    for (const vs of subject.vertices()) {
        for (const vc of clip.vertices()) {
            if (Math.abs(vs.point.x - vc.point.x) < tolerance &&
 ...
 */
export function removeDuplicateVertices(polygon) {
    if (!polygon.first || polygon.count < 2)
        return;
...
//# sourceMappingURL=degenerate.js.map