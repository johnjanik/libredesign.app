...
export class Polygon {
  ...
  *vertices(): Generator<Vertex> {
    ...
    let current: Vertex | null = this.first;
    do {
      yield current!;
      current = current!.next;
  ...
  *edges(): Generator<[Vertex, Vertex]> {
    if (!this.first || this._count < 2) return;
  ...
  *unvisitedIntersections(): Generator<Vertex> {
  ...
  clone(): Polygon {
    ...
    for (const v of this.vertices()) {
      ...
      if (v.next) copyV.next = vertexMap.get(v.next) || null;
      if (v.prev) copyV.prev = vertexMap.get(v.prev) || null;
      if (v.neighbor) copyV.neighbor = vertexMap.get(v.neighbor) || null;
    }
 ...
 */
export function createPolygonsFromPath(
  path: VectorPath,
  source: 'subject' | 'clip' = 'subject',
 ...
 */
export function polygonToPathCommands(polygon: Polygon): PathCommand[] {
 ...
 */
export function polygonsToVectorPath(
  polygons: Polygon[],
