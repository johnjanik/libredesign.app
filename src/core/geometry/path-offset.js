 ...
 */
export function offsetPath(path, config) {
 ...
 */
function offsetPolygon(points, distance, joinStyle, miterLimit) {
 ...
 */
function addArc(result, center, radius, startAngle, endAngle, ccw) {
    // Normalize angles
 ...
 */
function flattenPathToPolygons(path, tolerance) {
    const polygons = [];
    let currentPolygon = [];
    ...
    for (const cmd of path.commands) {
        switch (cmd.type) {
            ...
            case 'C':
                ...
                const flatPoints = flattenBezier(currentPoint, { x: cmd.x1, y: cmd.y1 }, { x: cmd.x2, y: cmd.y2 }, { x: cmd.x, y: cmd.y }, tolerance);
 ...
 */
function flattenBezier(p0, p1, p2, p3, tolerance) {
    const result = [p0];
 ...
 */
function polygonToVectorPath(points, windingRule) {
    if (points.length === 0) {
 ...
 */
export function offsetForStrokeAlignment(path, strokeWeight, alignment, joinStyle = 'MITER', miterLimit = DEFAULT_MITER_LIMIT) {
    switch (alignment) {
        case 'INSIDE':
 ...
 */
export function createStrokeOutline(path, strokeWeight, joinStyle = 'MITER', miterLimit = DEFAULT_MITER_LIMIT) {
...
//# sourceMappingURL=path-offset.js.map