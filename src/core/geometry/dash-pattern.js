 ...
 */
export function applyDashPattern(path, config) {
    ...
    const dashedPaths = generateDashedPaths(segments, normalizedPattern, offset, patternLength, path.windingRule);
    return { paths: dashedPaths, totalLength };
}
/**
 * Calculate the total length of a path
 */
export function pathLength(path, tolerance = DEFAULT_TOLERANCE) {
    const segments = pathToSegments(path, tolerance);
 ...
 */
export function pointAtLength(path, targetLength, tolerance = DEFAULT_TOLERANCE) {
    const segments = pathToSegments(path, tolerance);
 ...
 */
export function tangentAtLength(path, targetLength, tolerance = DEFAULT_TOLERANCE) {
    const segments = pathToSegments(path, tolerance);
    let accumulatedLength = 0;
    for (const segment of segments) {
        if (accumulatedLength + segment.length >= targetLength) {
 ...
 */
function pathToSegments(path, tolerance) {
    const segments = [];
    ...
    for (const cmd of path.commands) {
        switch (cmd.type) {
            ...
            case 'C':
                if (currentPoint) {
                    const curveSegments = flattenCubicBezier(currentPoint, { x: cmd.x1, y: cmd.y1 }, { x: cmd.x2, y: cmd.y2 }, { x: cmd.x, y: cmd.y }, tolerance);
...
function flattenBezierRecursive(p0, p1, p2, p3, tolerance, segments) {
 ...
 */
function generateDashedPaths(segments, pattern, offset, patternLength, windingRule) {
    const paths = [];
 ...
 */
function pointsToDashPath(points, windingRule) {
    if (points.length === 0) {
...
//# sourceMappingURL=dash-pattern.js.map