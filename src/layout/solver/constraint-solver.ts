 ...
 */
const STRENGTH_MAP: Record<ConstraintStrength, number> = {
...
export class ConstraintSolver {
  ...
  private variables: Map<string, LayoutVariable> = new Map();
  ...
  addRelationConstraint(
    id: string,
    variable1: kiwi.Variable,
    variable2: kiwi.Variable,
    offset: number = 0,
    strength: ConstraintStrength = 'required'
  ): void {
    // expression: variable1 - variable2 - offset == 0
    const expression = new kiwi.Expression(
      variable1,
      [-1, variable2],
      -offset
    );
   ...
   */
  private addConstraintInternal(id: string, constraint: kiwi.Constraint): void {
 ...
 */
export function createContainmentConstraints(
 ...
 */
export function createAlignmentConstraints(
  solver: ConstraintSolver,
  parentId: NodeId,
  childId: NodeId,
  ...
  switch (horizontalAlign) {
    ...
    case 'stretch':
      ...
      solver.addEquality(
        `${prefix}:h:width`,
        new kiwi.Expression(
          solver.width(childId),
          [-1, solver.width(parentId)]
        ),
        strength
      );
      break;
  ...
  switch (verticalAlign) {
    ...
    case 'stretch':
      ...
      solver.addEquality(
        `${prefix}:v:height`,
        new kiwi.Expression(
          solver.height(childId),
          [-1, solver.height(parentId)]
        ),
        strength
      );
      break;
 ...
 */
export function createSizeConstraints(
  solver: ConstraintSolver,
  nodeId: NodeId,
 ...
 */
export function createPositionConstraints(
  solver: ConstraintSolver,
  nodeId: NodeId,
