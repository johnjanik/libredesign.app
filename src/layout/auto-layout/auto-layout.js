 ...
 */
export function calculateAutoLayout(config) {
    const { mode, itemSpacing, paddingTop, paddingRight, paddingBottom, paddingLeft, primaryAxisAlignItems, counterAxisAlignItems, children, containerWidth, containerHeight, } = config;
 ...
 */
export function applyAutoLayoutConstraints(solver, containerId, config, strength = 'strong') {
    ...
    for (const result of results) {
        ...
        solver.addEquality(`${nodePrefix}:x`, new kiwi.Expression(solver.x(result.nodeId), [-1, solver.x(containerId)], -result.x), strength);
        solver.addEquality(`${nodePrefix}:y`, new kiwi.Expression(solver.y(result.nodeId), [-1, solver.y(containerId)], -result.y), strength);
        // Size constraints
        solver.addValueConstraint(`${nodePrefix}:w`, solver.width(result.nodeId), result.width, strength);
        solver.addValueConstraint(`${nodePrefix}:h`, solver.height(result.nodeId), result.height, strength);
    }
}
/**
 * Calculate the minimum size needed for an auto layout container.
 */
export function calculateAutoLayoutMinSize(config) {
    const { mode, itemSpacing, paddingTop, paddingRight, paddingBottom, paddingLeft, children, } = config;
    if (children.length === 0 || mode === 'NONE') {
        return {
 ...
 */
export function needsRelayout(config, oldSize, newSize) {
    ...
    if (crossAxisChanged) {
        ...
        const hasStretchChild = config.children.some((c) => {
 ...
 */
export function createAutoLayoutConfig(props, children, containerWidth, containerHeight) {
    return {
        mode: props.mode,
