...
export class CodePanel {
    ...
    updateCode() {
        ...
        if (this.selectedNodeIds.length === 0) {
            this.codeElement.innerHTML = this.highlightSyntax('/* Select an element to generate code */', 'css');
            return;
        }
    ...
    highlightSyntax(code, language) {
        ...
        if (language === 'css') {
            // CSS keywords
            highlighted = highlighted.replace(/\b(px|em|rem|%|vh|vw|solid|none|auto)\b/g, '<span style="color: #0066ff;">$1</span>');
            // Property names
            highlighted = highlighted.replace(/([a-z-]+)(?=\s*:)/g, '<span style="color: #9932cc;">$1</span>');
            // Values
            highlighted = highlighted.replace(/(#[a-fA-F0-9]{3,8})/g, '<span style="color: #098658;">$1</span>');
            // Comments
            highlighted = highlighted.replace(/(\/\*[\s\S]*?\*\/)/g, '<span style="color: #6a737d;">$1</span>');
        }
        else if (language === 'swift') {
            // Swift keywords
            highlighted = highlighted.replace(/\b(import|struct|var|let|func|return|some|View|body)\b/g, '<span style="color: #af00db;">$1</span>');
            // Types
            highlighted = highlighted.replace(/\b(Text|Rectangle|Color|SwiftUI)\b/g, '<span style="color: #267f99;">$1</span>');
            // Numbers
            highlighted = highlighted.replace(/\b(\d+\.?\d*)\b/g, '<span style="color: #098658;">$1</span>');
        }
        else if (language === 'kotlin') {
            // Kotlin keywords
            highlighted = highlighted.replace(/\b(import|fun|val|var|return|package)\b/g, '<span style="color: #af00db;">$1</span>');
            // Annotations
            highlighted = highlighted.replace(/(@\w+)/g, '<span style="color: #795e26;">$1</span>');
            // Types
            highlighted = highlighted.replace(/\b(Modifier|Color|Box|Text|Composable)\b/g, '<span style="color: #267f99;">$1</span>');
        }
        return highlighted;
    }
    escapeHtml(text) {
 ...
 * Create a code panel.
 */
export function createCodePanel(runtime, container, options) {
