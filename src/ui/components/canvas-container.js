...
export class CanvasContainer {
     ...
     */
    renderSelectionOverlay(ctx, scaleX, scaleY) {
        const selectionManager = this.runtime.getSelectionManager();
        const sceneGraph = this.runtime.getSceneGraph();
        const viewport = this.runtime.getViewport();
        if (!selectionManager || !sceneGraph || !viewport)
            return;
    ...
    drawPixelGrid(ctx) {
        ...
        if (zoom < 8)
            return; // Only show grid at high zoom
        const gridSize = zoom;
 ...
 */
export function createCanvasContainer(runtime, container, options) {
    return new CanvasContainer(runtime, container, options);
}
//# sourceMappingURL=canvas-container.js.map