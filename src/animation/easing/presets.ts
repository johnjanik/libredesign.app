...
function reverseEasingInternal(easing: EasingFunction): EasingFunction {
  return (t) => 1 - easing(1 - t);
}

function mirrorEasingInternal(easing: EasingFunction): EasingFunction {
  return (t) => {
    if (t < 0.5) {
      return easing(t * 2) / 2;
    } else {
...
const elasticOutPreset: EasingFunction = (t) => {
  if (t === 0) return 0;
  if (t === 1) return 1;
  const amplitude = 1;
  const period = 0.3;
...
const bounceOutPreset: EasingFunction = (t) => {
  const n1 = 7.5625;
  const d1 = 2.75;
...
export function resolveEasing(
  easing: EasingPreset | EasingDefinition | EasingFunction
): EasingFunction {
  // Already a function
 ...
 */
export function createOvershootEasing(
 ...
 */
export function createBounceEasing(): EasingFunction {
  return (t) => {
    const n1 = 7.5625;
    const d1 = 2.75;
