 ...
 */
export function createStepsEasing(steps, position = 'end') {
    if (steps < 1) {
        throw new Error('Steps must be at least 1');
    }
    // Normalize position aliases
    const normalizedPosition = normalizePosition(position);
    return function stepsEasing(t) {
        if (t <= 0)
            return getStartValue(normalizedPosition, steps);
        if (t >= 1)
            return getEndValue(normalizedPosition, steps);
 ...
 */
function getStartValue(position, steps) {
    switch (position) {
        case 'jump-start':
 ...
 */
function getEndValue(position, steps) {
    switch (position) {
        case 'jump-end':
        case 'jump-both':
            return 1;
 ...
 */
function computeStepValue(t, steps, position) {
    let currentStep;
    let totalSteps;
    switch (position) {
        case 'jump-start':
