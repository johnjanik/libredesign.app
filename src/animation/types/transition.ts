...
export interface OverlaySettings {
  ...
  readonly position: 'CENTER' | 'TOP_LEFT' | 'TOP_CENTER' | 'TOP_RIGHT' |
 ...
 */
export function createPrototypeLink(
  sourceNodeId: NodeId,
  targetNodeId: NodeId,
  options: {
    trigger?: TriggerType;
  ...
  if (options.direction !== undefined) {
    (link as { direction: TransitionDirection }).direction = options.direction;
  }
  if (options.delay !== undefined) {
    (link as { delay: number }).delay = options.delay;
  }
  return link;
}
