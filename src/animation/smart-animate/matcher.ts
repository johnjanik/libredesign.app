...
export function matchNodes(
  ...
  const matchedSourceIds = new Set<NodeId>();
  const matchedTargetIds = new Set<NodeId>();
  const matches: NodeMatch[] = [];

  // Create lookup maps
  const sourceById = new Map<NodeId, NodeData>();
  const targetById = new Map<NodeId, NodeData>();
  const sourceByName = new Map<string, NodeData[]>();
  const targetByName = new Map<string, NodeData[]>();
  ...
  if (opts.matchByName) {
    for (const sourceNode of sourceNodes) {
      if (matchedSourceIds.has(sourceNode.id)) continue;
      ...
      let bestMatch: { node: NodeData; score: number } | null = null;

      for (const candidate of candidates) {
        if (matchedTargetIds.has(candidate.id)) continue;
        if (candidate.type !== sourceNode.type) continue;
  ...
  if (opts.matchByPosition) {
    for (const sourceNode of sourceNodes) {
      ...
      for (const targetNode of targetNodes) {
        if (matchedTargetIds.has(targetNode.id)) continue;
        if (targetNode.type !== sourceNode.type) continue;

        const score = calculatePositionMatchScore(
          sourceNode,
          targetNode,
          sceneGraph,
          opts
...
function calculateNameMatchScore(
  ...
  if (opts.matchByPosition && 'x' in source && 'x' in target) {
    const posScore = calculatePositionSimilarity(
      source as NodeData & { x: number; y: number },
      target as NodeData & { x: number; y: number },
...
function calculatePositionMatchScore(
  ...
  const sourcePos = source as NodeData & { x: number; y: number };
  const targetPos = target as NodeData & { x: number; y: number };
  ...
  if (posScore < 0.3) return 0;

  let score = posScore * 0.6;

  // Size similarity bonus
  if ('width' in source && 'width' in target) {
    const sizeScore = calculateSizeSimilarity(
      source as NodeData & { width: number; height: number },
      target as NodeData & { width: number; height: number }
    );
...
function calculatePositionSimilarity(
  ...
  if (distance <= tolerance * 0.1) return 1;
 ...
 */
function getParentNames(nodeId: NodeId, sceneGraph: SceneGraph): string[] {
  const names: string[] = [];
  ...
  while (currentId && depth < maxDepth) {
    ...
    if (!node || !('parentId' in node)) break;

    const parentId = (node as NodeData & { parentId?: NodeId }).parentId;
    if (!parentId) break;
 ...
 */
export function getAnimatableNodes(
  frameId: NodeId,
  sceneGraph: SceneGraph
): NodeData[] {
  ...
  function collectNodes(nodeId: NodeId): void {
 ...
 */
function isAnimatableNode(node: NodeData): boolean {
