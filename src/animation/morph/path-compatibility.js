 ...
 */
export function checkCompatibility(sourcePath, targetPath, options = {}) {
    ...
    const { sourceSubdivisions, targetSubdivisions } = calculateSubdivisions(sourcePoints, targetPoints);
    return {
        level: 'subdivide',
        canMorph: true,
        sourcePoints,
 ...
 */
export function countPoints(path) {
    let count = 0;
 ...
 */
function calculateSubdivisions(sourcePoints, targetPoints) {
    if (sourcePoints === targetPoints) {
 ...
 */
export function getCommandTypes(path) {
    const types = new Set();
    for (const cmd of path.commands) {
        types.add(cmd.type);
    }
 ...
 */
export function hasCompatibleCommands(sourcePath, targetPath) {
...
//# sourceMappingURL=path-compatibility.js.map