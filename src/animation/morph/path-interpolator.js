 ...
 */
export function prepareMorph(sourcePath, targetPath, options = {}) {
    ...
    const { source: normalizedSource, target: normalizedTarget } = normalizePathPoints(sourcePath, targetPath);
 ...
 */
function createSnapTransition(sourcePath, targetPath) {
    ...
    return {
        ...
        isCompatible: false,
        interpolate: (t) => (t < 0.5 ? sourcePath : targetPath),
    };
}
/**
 * Interpolate between two paths at a given progress.
 */
function interpolatePaths(sourcePath, targetPath, matching, t) {
    // Handle edge cases
    if (t <= 0)
        return sourcePath;
    if (t >= 1)
 ...
 */
function getTargetCommand(targetCommands, matching, matchIndex, commandIndex) {
 ...
 */
function interpolateCommand(source, target, t) {
    // Handle Z commands
 ...
 */
function getInterpolatedCommandType(sourceType, targetType) {
 ...
 */
export function interpolatePathArrays(sourcePaths, targetPaths, t, options = {}) {
 ...
 */
export function createMorphAnimator(sourcePaths, targetPaths, options = {}) {
...
//# sourceMappingURL=path-interpolator.js.map