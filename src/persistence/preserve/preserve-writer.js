...
import { PRESERVE_MIMETYPE, PRESERVE_FORMAT_VERSION, } from './preserve-types';
...
export class PreserveWriter {
    ...
    options;
    entries = [];
    constructor(sceneGraph, tokenRegistry = null, options = {}) {
        this.zip = new JSZip();
    ...
    writePages() {
        ...
        for (const pageId of pageIds) {
            const pageNode = this.sceneGraph.getNode(pageId);
            if (!pageNode)
                continue;
    ...
    addEntry(path, type, size, mediaType) {
 ...
 */
export async function createPreserveArchive(sceneGraph, tokenRegistry = null, options = {}) {
    const writer = new PreserveWriter(sceneGraph, tokenRegistry, options);
    return writer.write();
}
//# sourceMappingURL=preserve-writer.js.map