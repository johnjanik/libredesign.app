...
export class SVGImporter {
    ...
    calculateViewBoxTransform(viewBox, dimensions, scale) {
        if (!viewBox) {
    ...
    parseLength(value) {
        ...
        if (value.endsWith('em'))
            return num * 16;
        if (value.endsWith('%'))
            return num; // Would need context
        return num;
    }
    // =========================================================================
    // Private Methods - Importing
    // =========================================================================
    importChildren(element, parentId, transform, options) {
        for (const child of Array.from(element.children)) {
            this.importElement(child, parentId, transform, options);
        }
    }
    importElement(element, parentId, transform, options) {
    ...
    importGroup(element, parentId, transform, options) {
        // Get group transform
    ...
    importRect(element, parentId, transform) {
    ...
    importCircle(element, parentId, transform) {
    ...
    importEllipse(element, parentId, transform) {
        const cx = this.parseLength(element.getAttribute('cx') ?? '0') * transform.scaleX + transform.translateX;
        const cy = this.parseLength(element.getAttribute('cy') ?? '0') * transform.scaleY + transform.translateY;
        const rx = this.parseLength(element.getAttribute('rx') ?? '0') * transform.scaleX;
    ...
    importLine(element, parentId, transform) {
    ...
    importPolyline(element, parentId, transform, closed) {
    ...
    importPath(element, parentId, transform) {
        ...
        const normalizedCommands = commands.map(cmd => {
            switch (cmd.type) {
                case 'M':
                    return { type: 'M', x: cmd.x - bounds.minX, y: cmd.y - bounds.minY };
                case 'L':
                    return { type: 'L', x: cmd.x - bounds.minX, y: cmd.y - bounds.minY };
                case 'C':
                    return {
                        type: 'C',
        ...
        const path = { windingRule, commands: normalizedCommands };
        const { fills, strokes, strokeWeight } = this.parseStyle(element, transform);
        this.sceneGraph.createNode('VECTOR', parentId, -1, {
    ...
    importText(element, parentId, transform) {
        const x = this.parseLength(element.getAttribute('x') ?? '0') * transform.scaleX + transform.translateX;
        const y = this.parseLength(element.getAttribute('y') ?? '0') * transform.scaleY + transform.translateY;
    ...
    importImage(element, parentId, transform) {
        const x = this.parseLength(element.getAttribute('x') ?? '0') * transform.scaleX + transform.translateX;
        const y = this.parseLength(element.getAttribute('y') ?? '0') * transform.scaleY + transform.translateY;
        const width = this.parseLength(element.getAttribute('width') ?? '0') * transform.scaleX;
        const height = this.parseLength(element.getAttribute('height') ?? '0') * transform.scaleY;
            ...
            switch (cmd) {
                ...
                case 'm': {
                    ...
                    while (i < tokens.length && typeof tokens[i] === 'number') {
                        const x = tokens[i++] * transform.scaleX + (isRelative ? currentX : 0) + transform.translateX;
                        const y = tokens[i++] * transform.scaleY + (isRelative ? currentY : 0) + transform.translateY;
                ...
                case 'l': {
                    ...
                    while (i < tokens.length && typeof tokens[i] === 'number') {
                        const x = tokens[i++] * transform.scaleX + (isRelative ? currentX : transform.translateX);
                        const y = tokens[i++] * transform.scaleY + (isRelative ? currentY : transform.translateY);
                ...
                case 'c': {
                    ...
                    while (i < tokens.length && typeof tokens[i] === 'number') {
                        const x1 = tokens[i++] * transform.scaleX + (isRelative ? currentX : transform.translateX);
                        const y1 = tokens[i++] * transform.scaleY + (isRelative ? currentY : transform.translateY);
                        const x2 = tokens[i++] * transform.scaleX + (isRelative ? currentX : transform.translateX);
                        const y2 = tokens[i++] * transform.scaleY + (isRelative ? currentY : transform.translateY);
                        const x = tokens[i++] * transform.scaleX + (isRelative ? currentX : transform.translateX);
                        const y = tokens[i++] * transform.scaleY + (isRelative ? currentY : transform.translateY);
                ...
                case 'q': {
                    ...
                    while (i < tokens.length && typeof tokens[i] === 'number') {
                        const qx = tokens[i++] * transform.scaleX + (isRelative ? currentX : transform.translateX);
                        const qy = tokens[i++] * transform.scaleY + (isRelative ? currentY : transform.translateY);
                        const x = tokens[i++] * transform.scaleX + (isRelative ? currentX : transform.translateX);
                        const y = tokens[i++] * transform.scaleY + (isRelative ? currentY : transform.translateY);
    ...
    // =========================================================================
    createEllipsePath(_cx, _cy, rx, ry) {
 ...
 */
export function createSVGImporter(sceneGraph) {
    return new SVGImporter(sceneGraph);
}
//# sourceMappingURL=svg-importer.js.map