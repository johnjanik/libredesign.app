 ...
 */
import { DocumentSerializer } from '../serialization/document-serializer';
...
class ZipWriter {
    ...
    generate() {
        ...
        const endRecord = this.createEndOfCentralDirectory(this.files.length, offset - centralDirOffset, centralDirOffset);
    ...
    createCentralDirectoryHeader(file, localHeaderOffset) {
        const nameBytes = new TextEncoder().encode(file.name);
...
export class DesignLibreBundler {
    ...
    async bundle(options = {}) {
        ...
        // Collect images
        const images = includeImages ? this.collectImages() : new Map();
        let imageCount = 0;
    ...
    async import(file) {
        ...
        const document = JSON.parse(new TextDecoder().decode(canvasData));
        // Read images
        ...
        for (const [name, data] of files) {
            if (name.startsWith('images/')) {
                ...
                images.set(imageName, new Blob([data], { type: mimeType }));
            }
        }
        // Read fonts
        const fonts = new Map();
        for (const [name, data] of files) {
            if (name.startsWith('fonts/')) {
                ...
                fonts.set(fontName, new Blob([data], { type: 'font/ttf' }));
            }
        }
        // Read thumbnail
        let thumbnail = null;
        ...
        if (thumbnailData) {
            thumbnail = new Blob([thumbnailData], { type: 'image/png' });
        }
        return {
            meta,
            document,
     ...
     */
    apply(importResult) {
        this.serializer.deserialize(JSON.stringify(importResult.document), this.sceneGraph, { validate: true });
    }
    /**
     * Download the bundle.
     */
    async download(filename = 'document.designlibre', options = {}) {
    ...
    async getThumbnail(file) {
        ...
        return new Blob([thumbnailData], { type: 'image/png' });
    }
    // =========================================================================
    // Private Methods
    // =========================================================================
 ...
 */
export function createDesignLibreBundler(sceneGraph) {
    return new DesignLibreBundler(sceneGraph);
}
//# sourceMappingURL=designlibre-bundler.js.map