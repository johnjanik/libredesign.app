 ...
 */
export async function copyMultiple(items, format = 'plain') {
    let text;
    switch (format) {
        case 'json':
            text = JSON.stringify(Object.fromEntries(items.map(item => [item.label, item.value])), null, 2);
            break;
        case 'css':
 ...
 */
export async function copyCodeAsHTML(code, highlightedHTML) {
 ...
 */
export function showCopyFeedback(element, success, duration = 1500) {
 ...
 */
export function createCopyButton(textToCopy, options = {}) {
