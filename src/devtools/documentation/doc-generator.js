...
export class DocumentationGenerator {
    sceneGraph;
    tokenRegistry;
    constructor(sceneGraph, tokenRegistry) {
     ...
     */
    generateComponentDoc(componentId) {
        ...
        for (const childId of childIds) {
            ...
            if (child.type === 'FRAME') {
                ...
                if (doc) {
                    ...
                    componentsSection.components.push(doc);
                }
            }
        }
        return {
    ...
    exportToHTML(doc) {
        ...
        if (doc.usedTokens.length > 0) {
            ...
            for (const token of doc.usedTokens) {
                ...
                let colorSwatch = '';
                if (token.type === 'color') {
                    const hex = rgbaToHex(token.value);
     ...
     */
    exportStyleGuideToMarkdown(guide) {
        const lines = [];
                        ...
                        if (fill.type === 'SOLID') {
                            const matchingToken = allTokens.find(t => {
                                if (t.type !== 'color')
                                    return false;
                                const tc = t.value;
                                return (Math.abs(tc.r - fill.color.r) < 0.01 &&
 ...
 */
export function createDocumentationGenerator(sceneGraph, tokenRegistry) {
