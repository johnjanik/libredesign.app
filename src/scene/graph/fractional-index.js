 ...
 */
export function generateFirstIndex() {
    return MID_CHAR;
}
/**
 ...
 */
export function generateIndexBefore(first) {
...
export function generateIndexAfter(last) {
    ...
    // Otherwise, append a middle character
    return (last + MID_CHAR);
}
/**
 * Generate an index between two existing indices.
 * Guarantees: before < result < after
 */
export function generateIndexBetween(before, after) {
    // Validate ordering
    if (before >= after) {
        throw new Error(`Invalid index ordering: before "${before}" must be less than after "${after}"`);
    }
    // Find the common prefix length
    let commonLength = 0;
    while (commonLength < before.length &&
        commonLength < after.length &&
        before[commonLength] === after[commonLength]) {
        commonLength++;
    }
    // Get the characters at the divergence point
    const beforeCode = commonLength < before.length
        ? before.charCodeAt(commonLength)
        : MIN_CODE;
    const afterCode = commonLength < after.length
    ...
    if (afterCode - beforeCode > 1) {
        ...
        return (before.slice(0, commonLength) + String.fromCharCode(midCode));
    }
    ...
    const suffixStart = commonLength + 1 < before.length
        ? before.charCodeAt(commonLength + 1)
        : MIN_CODE;
    const midSuffix = Math.floor((suffixStart + MAX_CODE) / 2);
    return (prefix + String.fromCharCode(midSuffix));
}
/**
 ...
 */
export function generateIndexAtPosition(sortedIndices, position) {
    ...
    return generateIndexBetween(sortedIndices[position - 1], sortedIndices[position]);
}
/**
 * Compare two fractional indices.
 * Returns negative if a < b, positive if a > b, zero if equal.
 */
export function compareFractionalIndices(a, b) {
    return a.localeCompare(b);
}
/**
 * Sort an array of items by their fractional indices.
 */
export function sortByFractionalIndex(items, getIndex) {
    return [...items].sort((a, b) => compareFractionalIndices(getIndex(a), getIndex(b)));
}
/**
 * Validate that a fractional index is well-formed.
 */
export function isValidFractionalIndex(index) {
    if (index.length === 0)
        return false;
 ...
 */
export function generateEvenlySpacedIndices(count) {
    if (count === 0)
        return [];
    if (count === 1)
        return [generateFirstIndex()];
    ...
    for (let i = 1; i <= count; i++) {
        ...
        indices.push(String.fromCharCode(code));
    }
    return indices;
}
