...
import { createOverrideResolver } from './override-resolver';
...
export class ComponentManager extends EventEmitter {
     ...
     */
    createComponentFromNode(sourceNodeId, options = {}) {
        ...
        this.registry.registerComponent(componentId, options.name ?? sourceNode.name, options.componentSetId ?? null, options.variantKey ?? null);
     ...
     */
    createEmptyComponent(parentId, options = {}) {
        const componentId = this.sceneGraph.createNode('COMPONENT', parentId, -1, {
            name: options.name ?? 'Component',
        });
        this.registry.registerComponent(componentId, options.name ?? 'Component', options.componentSetId ?? null, options.variantKey ?? null);
        if (options.componentSetId && options.variantKey) {
            this.registry.addToComponentSet(options.componentSetId, options.variantKey, componentId);
        }
    ...
    updateComponent(componentId) {
        if (!this.registry.hasComponent(componentId))
            return;
     ...
     */
    createInstance(componentId, parentId, options = {}) {
     ...
     */
    isPropertyOverridden(instanceId, path) {
        const instance = this.sceneGraph.getNode(instanceId);
        if (!instance || instance.type !== 'INSTANCE')
            return false;
     ...
     */
    getResolvedInstanceData(instanceId) {
        const instance = this.sceneGraph.getNode(instanceId);
        if (!instance || instance.type !== 'INSTANCE')
            return null;
     ...
     */
    getEffectiveValue(instanceId, path) {
        const instance = this.sceneGraph.getNode(instanceId);
        if (!instance || instance.type !== 'INSTANCE')
            return undefined;
        const component = this.sceneGraph.getNode(instance.componentId);
     ...
     */
    createComponentSet(name, variantProperties, componentIds) {
     ...
     */
    switchInstanceVariant(instanceId, newVariantKey) {
        const instance = this.sceneGraph.getNode(instanceId);
        if (!instance || instance.type !== 'INSTANCE')
            return false;
        const currentComponent = this.registry.getComponent(instance.componentId);
        if (!currentComponent?.componentSetId)
            return false;
        const newComponentId = this.registry.getVariantComponent(currentComponent.componentSetId, newVariantKey);
        if (!newComponentId)
            return false;
    ...
    extractNodeProperties(node) {
        const { id, type, parentId, childIds, ...rest } = node;
        return rest;
    }
    /**
     * Clone children from source to destination, applying overrides.
     */
    cloneChildren(sourceId, destId, overrides) {
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            // Create cloned node
            const clonedId = this.sceneGraph.createNode(child.type, destId, i, this.extractNodeProperties(resolvedChild));
     ...
     */
    parseVariantKey(component, properties) {
     ...
     */
    filterCompatibleOverrides(overrides, componentId) {
        const component = this.sceneGraph.getNode(componentId);
        if (!component)
            return [];
 ...
 */
export function createComponentManager(sceneGraph) {
