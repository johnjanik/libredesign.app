...
export class ImageRenderer {
    ...
    render(config) {
        this.checkDisposed();
        const { texture, scaleMode, transform = IDENTITY_MATRIX, opacity = 1, bounds, } = config;
        ...
        const uvBounds = this.calculateUVBounds(bounds[2], bounds[3], texture.width, texture.height, scaleMode);
     ...
     */
    renderWithUV(texture, bounds, uvBounds, transform = IDENTITY_MATRIX, opacity = 1) {
        this.checkDisposed();
        const gl = this.ctx.gl;
        const program = this.shaders.use('image');
        if (!program)
    ...
    calculateUVBounds(boundsWidth, boundsHeight, imageWidth, imageHeight, scaleMode) {
    ...
    setUniforms(program, transform, opacity, tiling) {
        const gl = this.ctx.gl;
        // Texture sampler
        ...
        if (uTransform !== undefined) {
            ...
            gl.uniformMatrix3fv(uTransform, false, new Float32Array([
                transform[0],
                transform[1],
                0,
                transform[2],
 ...
 */
export function createImageRenderer(ctx, shaders) {
