...
export class ImageCache {
   ...
   */
  async loadImage(url: string): Promise<ImageEntry> {
    this.checkDisposed();
   ...
   */
  async loadImageFromBlob(key: string, blob: Blob): Promise<ImageEntry> {
    this.checkDisposed();
   ...
   */
  async loadImageFromImageData(key: string, imageData: ImageData): Promise<ImageEntry> {
    this.checkDisposed();
   ...
   */
  getStats(): { count: number; pending: number; maxSize: number } {
    return {
  ...
  private async doLoadImage(url: string, signal: AbortSignal): Promise<ImageEntry> {
    try {
      let image: HTMLImageElement | ImageBitmap;
      let width: number;
      let height: number;
  ...
  private maybeEvict(): void {
    if (this.images.size <= this.maxCacheSize) return;
