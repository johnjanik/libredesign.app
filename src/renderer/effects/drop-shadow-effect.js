...
export class DropShadowEffectRenderer {
    ...
    apply(source, config, releaseSource = true) {
        ...
        gl.uniform2f(shadowShader.uniforms.get('uOffset'), config.offset.x / source.width, config.offset.y / source.height);
        gl.uniform4f(shadowShader.uniforms.get('uColor'), config.color.r, config.color.g, config.color.b, config.color.a);
 ...
 */
export function createDropShadowEffectRenderer(ctx, shaders, pool, options) {
