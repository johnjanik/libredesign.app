...
import type { Effect, BlurEffect, DropShadowEffect, InnerShadowEffect, BackgroundBlurEffect } from '@core/types/effect';
import type { WebGLContext } from '../core/webgl-context';
import type { ShaderManager } from '../shaders/shader-manager';
import { RenderTarget } from './render-target';
import { RenderTargetPool, createRenderTargetPool } from './render-target-pool';
...
export class EffectPipeline {
  ...
  private contextStack: EffectContext[] = [];
   ...
   */
  private calculatePadding(effects: Effect[]): number {
    let maxPadding = 0;
   ...
   */
  private applyBlur(source: RenderTarget, effect: BlurEffect): RenderTarget {
  ...
  private applyDropShadow(
    ...
    gl.uniform2f(
      ...
      effect.offset.x / source.width,
      effect.offset.y / source.height
    );
    gl.uniform4f(
 ...
 */
export function createEffectPipeline(
  ctx: WebGLContext,
  shaders: ShaderManager,
  config?: EffectPipelineConfig
): EffectPipeline {
